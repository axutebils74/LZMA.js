...
LZMA.oStream.prototype.writeBytes = function writeBytes(buffer, size)
{
	...
	this.buffers.push(new Uint8Array(buffer.subarray(0, size)));
	this.size += size;
}
LZMA.decompressFile = function(buffer){
  ...
  if ( !decoder.decodeBody(inStream, outStream, maxSize) ){throw Error}
  var buffers = outStream.buffers;
  var continous = new Uint8Array(outStream.size);
